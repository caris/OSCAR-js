...
<!--
	To be used in conjunction the WCS Download Protocol, not the SFV download widget. 
-->
...
<xsl:template match="owcs:Reference"  xmlns:owcs="http://www.opengis.net/wcs/1.1/ows" xmlns:xlink="http://www.w3.org/1999/xlink">
    <xsl:variable name="role">
        <xsl:choose>
            <xsl:when test="(contains(@xlink:role,'coverage'))">ui-icon-wcs-download</xsl:when>
			<xsl:when test="(contains(@xlink:role,'record'))">>ui-icon-document</xsl:when>
			<xsl:when test="(contains(@xlink:role,'BDB/attributes'))">>ui-icon-document</xsl:when>
            <xsl:otherwise>>ui-icon-wcs-download</xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
