...
oscar.ox.Theme = oscar
		.BaseClass({
			...
			hasSelectionService : function() {
				return (this.services && this.services.selection) ? true
						: false;
			},
			/**
			...
			getCatalogueService : function() {
				if (this.hasCatalogueService()) {
					...
					for (var i = 0; i < this.services.catalogue.serviceEntries.length; i++) {
						services.push(new oscar.ogc.CatalogueService(
								this.services.catalogue.serviceEntries[i]));
					}
					return services;
				} else {
					return [];
			...
			hasCatalogueService : function() {
				return (this.services && this.services.catalogue) ? true
						: false;
			},
			/**
			...
			hasExtractionService : function() {
				return (this.services && this.services.extraction) ? true
						: false;
			},
			/**
			...
			/**
			 * Method: buildMap Calls the toMapLayer for each of the layers in
			 * the theme to be added to the map.
			 * 
			...
			buildMap : function(map) {
				if (this.displaySRS) {
					try {
						var mPosition = map
								.getControlsByClass("OpenLayers.Control.MousePosition")[0];
						mPosition.displayProjection = new OpenLayers.Projection(
								this.displaySRS);
...
oscar.ox.Layer = oscar
		.BaseClass({
			...
			/**
			 * APIMethod: addDataLayer Adds an {<oscar.ox.DataLayer>} object to
			 * the dataLayers array.
			 * 
			 ...
			 * 
			 * Returns: {Object} *olLayer* which inherits from
			 * <OpenLayers.Layer> and is able to be added to an <OpenLayers.Map>
			 * object.
			 */
			toMapLayer : function(map) {
				...
				case "MARKER":
					...
					case "georss_simple":
						...
						olLayer = new OpenLayers.Layer.Vector(
								...
								{
									setMap : function() {
										OpenLayers.Layer.Vector.prototype.setMap
												...
												format : new OpenLayers.Format.GeoRSS(
														{
															...
															externalProjection : new OpenLayers.Projection(
																	"EPSG:4326"),
															createFeatureFromItem : function(
																	item) {
																var feature = OpenLayers.Format.GeoRSS.prototype.createFeatureFromItem
																		.apply(
																				this,
																				arguments);
																return feature;
						...
						olLayer.events
								.on({
									"featureselected" : function(evt) {
										...
										var pHolder = document
												.createElement("div");
										var header = document
												.createElement("div");
										header.innerHTML = feature.attributes['title'];
										var p = document.createElement("p");
										p.innerHTML = oscar.Util
												.parseText(feature.attributes['description']);
										pHolder.appendChild(header);
										pHolder.appendChild(p);
										html = p.innerHTML;
										var popup = new oscar.FramedCloud("id",
												feature.geometry.getBounds()
														.getCenterLonLat(),
												null, pHolder.innerHTML, null,
												true);
										popup.autoSize = true;
										feature.popup = popup;
						...
						olLayer.events
								.on({
									...
									"featureselected" : function(evt) {
										...
										var header = document
												.createElement("div");
										header.innerHTML = oscar.Util
												.parseText(feature.attributes['name']);
										var p = document.createElement("p");
										p.innerHTML = oscar.Util
												.parseText(feature.attributes['description']);
										pHolder.appendChild(header);
					...
					case "youtube":
						onEvents = {
							"featureselected" : function(evt) {
								...
								var fid = feature.fid.substring(feature.fid
										.lastIndexOf("/") + 1);
								html = '<iframe width="480" height="390" src="'
										+ youtubeString
										+ fid
								...
								var popup = new oscar.FramedCloud("id",
										feature.geometry.getBounds()
												.getCenterLonLat(), null,
										pHolder.innerHTML, null, true);
								popup.autoSize = true;
								feature.popup = popup;
								feature.layer.map.addPopup(popup);
							},
				...
				case "WMS":
					...
					olLayer = new OpenLayers.Layer.WMS(this.name, this.urls,
							params, {
								wrapDateLine : false,
								buffer : 0,
					...
					olLayer = new OpenLayers.Layer.WMTS(
							{
								...
								tileOrigin : new OpenLayers.LonLat(
										this.tileOrigin[0], this.tileOrigin[1]),
								tileFullExtent : new OpenLayers.Bounds(
										this.tileFullExtent[0],
										this.tileFullExtent[1],
										this.tileFullExtent[2],
										this.tileFullExtent[3]),
								style : "default",
