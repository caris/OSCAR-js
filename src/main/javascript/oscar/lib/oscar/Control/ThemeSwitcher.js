...
oscar.Control.ThemeSwitcher = oscar.BaseClass(oscar.Control.DragPanel, {
	...
	changeTheme : function(val,options) {
        ...
        this.$themeName.html(this.ox.themes[val].name);
	...
	buildMenu : function() {
        ...
        this.$activeThemeName = $$("<div></div>").css({
            "cursor":"pointer",
            "font-weight":"bold",
            "font-size":"12px",
            "color":"#fff",
        ...
        var $sprite = $$("<div></div>").addClass("themeSpriteDropdown");
        this.$themeName = $$("<div></div>").addClass("themeSwitcherActiveTheme");
        this.$activeThemeName.append($sprite);
        this.$activeThemeName.append(this.$themeName);
        this.$themeMenu = $$("<ul></ul>").addClass("themeMenu");
        this.$themeMenu.mouseleave(function(){
            var s = this;
            setTimeout(function(){
                $$(s).fadeToggle();
            },0)
        });
        this.$themeDiv.append(this.$activeThemeName);
        this.$themeDiv.append(this.$themeMenu);
        $customContainer.append(this.$themeDiv);
		for ( var item in this.ox.themes) {
				var theme = this.ox.themes[item];
                var $li = $$("<li></li>").html(theme.name).css({
                    "cursor":"pointer",
                    "display":"block",
                    "padding":"4px"
                }).click(function() {
                    scope.changeTheme($$(this).attr("index"));
                }).attr("index",item).mouseover(function() {
                    $$(this).addClass("themeHover");
                }).mouseout(function() {
                    $$(this).removeClass("themeHover");
                });
                this.$themeMenu.append($li);
		}
        this.$activeThemeName.click(function() {
           scope.$themeMenu.fadeToggle();
        });
