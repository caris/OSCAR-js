...
oscar.Gui.DownloadOptions = oscar.BaseClass(oscar.Gui, {
	...
	buildDownloadOptions:function() {
		...
		oscar.jQuery(cropDiv).addClass("dOption crop");
		cropDiv.setAttribute("title",oscar.i18n("Crop"));
		buttonsPanel.appendChild(cropDiv);
		...
		OpenLayers.Event.observe(cropDiv, "click", function(e) {
	...
	makeCRSList:function(div,crss) {
		...
		oscar.jQuery(input).autocomplete({
            ...
            .append( "<a>" + item.code + "<br>" + item.description + "<br>" + item.urn + "</a>" )
            .appendTo( ul );
        ...
        input.value = crsReferences[0].code;
	...
	makeFieldList:function(div,fields) {
        ...
        $$(fieldDiv).css({position:'relative'});
